...
module system_top (
  ...
  OBUFT i_obuf_spi_clk (
   .T (~tx_output_enable),
   .I (spi_clk_s),
   .O (spi_clk));

  OBUFT i_obuf_spi_dio (
   .T (~tx_output_enable),
   .I (spi_dio_s),
   .O (spi_dio));
  ...
  assign tx_output_enable = gpio_o[52] & ~vadj_err;
 ...
 assign rx1_enable = ~tx_output_enable ? 1'bz : rx1_enable_s;
 assign rx2_enable = ~tx_output_enable ? 1'bz : rx2_enable_s;
 assign tx1_enable = ~tx_output_enable ? 1'bz : tx1_enable_s;
 assign tx2_enable = ~tx_output_enable ? 1'bz : tx2_enable_s;
